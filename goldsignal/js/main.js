...
function createScoreBarFill(score) {
  const tier = getScoreColor(score);
...
function buildRankingRow(stock, rank, { locked = false } = {}) {
...
function buildHeroRow(stock, rank) {
...
function renderRankingsTable(stocks, unlocked) {
...
function buildInvestorCard(investor) {
...
function renderHeroCard(stocks) {
...
function initEmailGate(stocks) {
  const gateForm = document.getElementById('gate-form');
  const gate = document.getElementById('email-gate');
  const emailInput = document.getElementById('gate-email');

  if (!gateForm || !gate) return;
...
function initPortfolioForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const email = form.querySelector('#review-email')?.value.trim() ?? '';
    const tickers = form.querySelector('#review-tickers')?.value.trim() ?? '';
    ...
    if (!isValidEmail(email)) {
      const emailEl = form.querySelector('#review-email');
...
function observeFadeIns(elements) {
